...
import { DependencyGuardian } from './dependencies.js'
...
export const Config = Schema.object({
  ...
  profileName: Schema.string().default('web'),
  profileDir: Schema.string().default(''),
  dependencyScanLimit: Schema.number().min(20).max(2000).step(10).default(500),
  dependencyRepairTimeoutMs: Schema.number().min(10000).max(600000).step(10000).default(120000),
})
...
export function apply(ctx, config) {
  ...
  const dependencies = new DependencyGuardian(config)
  ctx.effect(() => guardian.start())
  ctx.inject(['connection'], web => {
    web.connection.rpc.handle('/cordis-guardian', createWebHandler(guardian, dependencies))
  })
}
